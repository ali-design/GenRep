    ...
    for cost_idx in range(len(costs_list)):
        ...
        if os.path.exists(out_file) and os.path.exists(ap_out_file):
            ...
            logger.info('cost: {} Accuracy: {} mean:{}'.format(
                 cost, class_scores, class_scores.mean()))
            logger.info('Saving cls cost Accuracy to: {}'.format(ap_out_file))
            np.save(ap_out_file, np.array([class_scores.mean()]))
            logger.info('Saving Classifier model to: {}'.format(out_file))
            with open(out_file, 'wb') as fwrite:
                pickle.dump(clf, fwrite)


def main():
    parser = argparse.ArgumentParser(description='logistic model training')
    ...
    parser.add_argument('--output_path', type=str, default=None,
                        help="path where to save the trained Trained models models")
