...
def truncated_noise_sample_neighbors(batch_size=1, dim_z=128, truncation=1., seed=None):
    ...
    norm_1 = np.random.normal(0., 0.1, size=(batch_size, dim_z)).astype(np.float32)
    norm_2 = np.random.normal(0., 0.2, size=(batch_size, dim_z)).astype(np.float32)
    norm_3 = np.random.normal(0., 0.5, size=(batch_size, dim_z)).astype(np.float32)
    norm_4 = np.random.normal(0., 1., size=(batch_size, dim_z)).astype(np.float32)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--desc', default='steer_rndgaussian_100', type=str, help='this will be the tag of this specfic dataset, added to the end of the dataset name')
