...
def truncated_noise_sample_neighbors(batch_size=1, dim_z=128, truncation=1.0, seed=None, num_neighbors=20, scale=1.0):
...
def sample(opt):
    ...
    for key in tqdm(imagenet_class_index_keys):
        ...
        noise_vector_neighbors = truncated_noise_sample_neighbors(truncation=truncation,
                                                        batch_size=nimg,
                                                        seed=seed, num_neighbors=opt.num_neighbors, scale=opt.std)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--desc', default='gauss1_std1.0_100', type=str, help='this will be the tag of this specfic dataset, added to the end of the dataset name')
