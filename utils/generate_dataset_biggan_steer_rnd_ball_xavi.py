...
def std2uniform(std_normal):
    """ Calculate the bounds of uniform distribution to achieve an std 
        equal to std_normal (average 0)
    """
    # std = b - a / sqrt(12) --> b = std * sqrt(3)
    return math.sqrt(3) * std_normal
...
def truncated_noise_sample_neighbors(batch_size=1, dim_z=128, truncation=1., seed=None):
    ...
    norm_1 = np.random.uniform(-std2uniform(0.1), std2uniform(0.1), size=(batch_size, dim_z)).astype(np.float32)
    norm_2 = np.random.uniform(-std2uniform(0.2), std2uniform(0.2), size=(batch_size, dim_z)).astype(np.float32)
    norm_3 = np.random.uniform(-std2uniform(0.5), std2uniform(0.5), size=(batch_size, dim_z)).astype(np.float32)
    norm_4 = np.random.uniform(-std2uniform(1.0), std2uniform(1.0), size=(batch_size, dim_z)).astype(np.float32)
            ...
            for batch_start in range(0, nimg, batch_size):
                ...
                for i, im in enumerate(ims):
                    if ii == 0:
                        ...
                        im_name = 'seed%04d_sample%05d_0.1.%s' % (seed, batch_start+i, imformat)
                    elif ii == 2:
                        im_name = 'seed%04d_sample%05d_0.2.%s' % (seed, batch_start+i, imformat)
                    elif ii == 3:
                        im_name = 'seed%04d_sample%05d_0.5.%s' % (seed, batch_start+i, imformat)
                    elif ii == 4:
                        im_name = 'seed%04d_sample%05d_1.0.%s' % (seed, batch_start+i, imformat)
                    im.save(os.path.join(class_dir_name, im_name))

if __name__ == '__main__':
    parser = argparse.ArgumentParser("Sample from biggan")
    parser.add_argument('--out_dir', default='/data/vision/torralba/scratch/xavierpuig/ganclr_results/gan_samples', type=str)
    ...
    parser.add_argument('--desc', default='steer_rndball_100', type=str, help='this will be the tag of this specfic dataset, added to the end of the dataset name')
