...
def trans_func(single_image, transform):
      out = transform(single_image)
      return out
trans = transform.Compose([
      # convert to PIL image
      transforms.ToPILImage()
      # add other transformation here: resize, color, ....
])
pool = Pool(num_pool)  # num_pool is the number of parallel processes you want
func = functools.partial(trans_func, transform=trans)
output = pool.map(func, tuple(image_batches))
# stack the output images into an array...
