...
#         with open('./utils/imagenet100_class_index.json', 'rb') as fid: 
        ...
        ##fromz
        self.z_dict = {}
        with open('./new_z_dict_2.pkl', 'rb') as fid:
            self.z_dict = pickle.load(fid)
        self.z_dict_keys = list(self.z_dict.keys())
    
    ##fromz
    def shuffle_z_dict(self):
        random.shuffle(self.z_dict_keys)
...
#         anchor_out = 255 * ((anchor_out + 1.0)/2.0)
    ...
    def __getitem__(self, indices):
        ...
        ##fromz
        indices = [x % 130000 for x in indices]
        start_time = time.time()
        self.lazy_init_gan()
        truncation = 1.0
        std_scale = 0.2
...
#         zs = self.z_dict[self.z_dict_keys[indices]]['val']
        zs = [self.z_dict[self.z_dict_keys[idx]]['val'] for idx in indices]
#         print(np.array(zs).shape)
#         print([self.z_dict_keys[idx] for idx in indices])
...
#         with tf.device('/device:GPU:{}'.format(self.gpu_idx_current)):
        ...
        ## fromz: sanity check
        print('saving orig anchor')
        a_out = (256. / 2.) * (anchor_out + 1.)
        a_out = np.clip(a_out, 0, 255)
        a_out = a_out.astype(np.uint8) 
        a_out = PIL.Image.fromarray(a_out[0])
        a_out_name = [self.z_dict_keys[idx] for idx in indices][0].replace('.png','')
        a_out.save('./check_images/'+a_out_name+'.png')
...
#         seed = start_seed + 2 * idx + 1000
#         seed = None
#         state = None if seed is None else np.random.RandomState(seed)
#         ws = truncation * truncnorm.rvs(-2, 2, size=(batch_size, 120), scale=std_scale, random_state=state).astype(np.float32)
#         zs = zs + ws
        zs = [self.z_dict[self.z_dict_keys[idx]]['neighbor']['val'] for idx in indices]
#         print([self.z_dict[self.z_dict_keys[idx]]['neighbor']['name'] for idx in indices])
#         print(np.array(zs).shape)
...
#         with tf.device('/device:GPU:{}'.format(self.gpu_idx_current)):
        ...
        ## fromz: sanity check
        print('saving orig neighbor')
        a_out2 = (256. / 2.) * (anchor_out2 + 1.)
        a_out2 = np.clip(a_out2, 0, 255)
        a_out2 = a_out2.astype(np.uint8) 
        a_out2 = PIL.Image.fromarray(a_out2[0])
        a_out2_name = [self.z_dict[self.z_dict_keys[idx]]['neighbor']['name'] for idx in indices][0].replace('.png', '')
        a_out2.save('./check_images/'+a_out2_name+'.png')
...
#         print('loader spent time', time.time() - start_time)
        ...
        ##fromz
        print('saving transformed anchor')
        a_out = images_anchor.detach().cpu().numpy()
        a_out = (256. / 2.) * (a_out + 1.)
        a_out = np.clip(a_out, 0, 255)
        a_out = a_out.astype(np.uint8)
        a_out = np.transpose(a_out, [0, 2, 3, 1])
        a_out = PIL.Image.fromarray(a_out[0])
        a_out_name = [self.z_dict_keys[idx] for idx in indices][0].replace('.png', '')
        a_out.save('./check_images/'+a_out_name+'_after_tranform.png')
    
        print('saving transformed neighbor')
        a_out = images_anchor2.detach().cpu().numpy()
        a_out = (256. / 2.) * (a_out + 1.)
        a_out = np.clip(a_out, 0, 255)
        a_out = a_out.astype(np.uint8)
        a_out = np.transpose(a_out, [0, 2, 3, 1])
        a_out = PIL.Image.fromarray(a_out[0])
        a_out_name = [self.z_dict[self.z_dict_keys[idx]]['neighbor']['name'] for idx in indices][0].replace('.png', '')
        a_out.save('./check_images/'+a_out_name+'_after_tranform.png')
...
def main():
    ...
    for epoch in range(init_epoch, opt.epochs + 1):
        ...
        # train for one epoch
        ##framz
        train_loader.dataset.shuffle_z_dict()
        time1 = time.time()
